...
struct dinic {
    ...
    ll dfs(cint u, ll cap) {
        if(u == t) { return cap; }
        ll sum = 0; int v; vis[u] = 1;
        for(int i=cur[u]; i; i=nx[i]) {
            cur[u] = i; v = to[i];
            if(c[i] && !vis[v] && dis[v] == dis[u] + w[i]) {
                ll d = dfs(v, min(cap, c[i]));
                if(d) { c[i] -= d; c[i^1] += d; cap -= d; sum += d; }
                if(cap == 0) { break; }
            }
        }
        vis[u] = 0;
    ...
    pair<ll, ll> max_flow(int s, int t) {
        ...
        this->t = t;
        ll flow = 0, res = 0, r;
        while(bfs(s, t)) { r = dfs(s, inf_int); flow += r; res += r*dis[t]; }
        return {flow, res};
    }
} A;
...
void solve(cint T) {
    cin >> n >> m >> s >> t;
    int u, v, w, c;
    for(int i=1; i<=m; i++) { cin >> u >> v >> w >> c; A.add(u, v, c, w); }
    auto ans = A.max_flow(s, t);
